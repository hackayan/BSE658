...
```{r}
ggplot(data = state_data, mapping = aes(x=Date, y=Positive)) + geom_point()
```

### Alternate way to make the same plots -- with a label
```{r}
...
```{r}
ggplot(data = state_data, mapping = aes(x=Date, y=Positive)) + geom_point(alpha = 0.2)
...
## Histograms plots -- colored!!
```{r}
ggplot(data = state_data, mapping = aes(x=Positive)) + geom_histogram(bins = 5, color = "white")
...
```
```{r}
# want to know what colors you can use? Hit enter!
colors()
```
...
```{r}
ggplot(data = covid_data, mapping = aes(x=Negative)) + geom_histogram(bins = 10, color = "white", fill = "steelblue") + facet_wrap(~State, nrow = 6)

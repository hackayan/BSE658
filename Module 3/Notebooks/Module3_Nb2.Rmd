...
#Pearson's
#Spearman's: sleep = [1, 4, 3, 2]; grump = [rank order]
```
...
Let's make some correlation plots using ggcorrplot library 
```{r}
...
Now the correlation coefficient we get is different from the Pearson's correlation coefficient _r_ we got earlier. This new correlation coefficient that we got is called '**Spearman's Correlation Coefficient**', denoted by $\rho$.
